                   ...
                   uint8_t expected_tag)
{
    ...
    assert(jcf != NULL);
    assert(index < jcf->constant_pool.count);

    info = jcf->constant_pool.pool[index];
    assert(expected_tag == info->tag);

    // Print the constant.
    switch (info->tag) {
    case JCF_CONSTANT_Class:
            // Print the class.
            print_jcf_constant(jcf, ntohs(((struct jcf_cp_class_info *) info)->name_index), JCF_CONSTANT_Utf8);
            break;
    ...
    case JCF_CONSTANT_InterfaceMethodref:
             ...
             * separated by a '.'.
             */
            print_jcf_constant(jcf, ntohs(((struct jcf_cp_ref_info *) info)->class_index), JCF_CONSTANT_Class);

            fprintf(stdout, ".");
            print_jcf_constant(jcf, ntohs(((struct jcf_cp_ref_info *) info)->name_and_type_index), JCF_CONSTANT_NameAndType);
            break;
    case JCF_CONSTANT_NameAndType:
            // Print the name and type.
            print_jcf_constant(jcf, ntohs(((struct jcf_cp_nameandtype_info *) info)->name_index), JCF_CONSTANT_Utf8);
            printf(" ");
            print_jcf_constant(jcf, ntohs(((struct jcf_cp_nameandtype_info *) info)->descriptor_index), JCF_CONSTANT_Utf8);
            break;
    case JCF_CONSTANT_Utf8:
            // Print the UTF8.
            fprintf(stdout, "%s", ((struct jcf_cp_utf8_info *) info)->bytes);
            break;
    default:
...
process_jcf_header(struct jcf_state *jcf)
{
    ...
    // Read the header
    if (fread(&magic, sizeof(magic), 1, jcf->f) != 1) {
        return (-1);
    }
    magic = ntohl(magic);

    if (fread(&minor, sizeof(minor), 1, jcf->f) != 1) {
        return (-1);
    }
    minor = ntohs(minor);

    if (fread(&major, sizeof(major), 1, jcf->f) != 1) {
        return (-1);
    }
    major = ntohs(major);
...
process_jcf_constant_pool(struct jcf_state *jcf)
{
    ...
    jcf->constant_pool.pool = Malloc(sizeof(struct jcf_cp_info*) * constant_pool_count);

    // Read the constant pool.
    for (i = 1; i < constant_pool_count; i++) {
        // Read the constant pool info tag.
        if (fread(&tag, 1, 1, jcf->f) != 1) {
            return (-1);
        }
        printf("tag: %u\n", tag);
        ...
        switch (tag) {
            ...
            case JCF_CONSTANT_MethodType:
                ...
                fseek(jcf->f, -1, SEEK_CUR);
                jcf->constant_pool.pool[i] = Malloc(3);

                if (fread(jcf->constant_pool.pool[i], 3, 1, jcf->f) != 1) {
                    return (-1);
                }
                break;
            case JCF_CONSTANT_Fieldref:
            ...
            case JCF_CONSTANT_InvokeDynamic:
                ...
                fseek(jcf->f, -1, SEEK_CUR);
                jcf->constant_pool.pool[i] = Malloc(5);
                if (fread(jcf->constant_pool.pool[i], 5, 1, jcf->f) != 1) {
                    return (-1);
                }
                break;
            ...
            case JCF_CONSTANT_Double:
                ...
                jcf->constant_pool.pool[i] = Malloc(4);

                if (fread(jcf->constant_pool.pool[i], 4, 1, jcf->f) != 1) {
                    return (-1);
                }
                break;
            case JCF_CONSTANT_Utf8:
                ...
                length = ntohs(length);
                fseek(jcf->f, -3, SEEK_CUR);
                jcf->constant_pool.pool[i] = Malloc(3 + length + 1);
                if (fread(jcf->constant_pool.pool[i], 3 + length, 1, jcf->f) != 1) {
                    return (-1);
                }
                jcf->constant_pool.pool[i] += '\0';
                break;
    ...
    if (jcf->depends_flag) {
        for (j = 1; j < constant_pool_count; j++) {
            tag =  jcf->constant_pool.pool[j]->tag;
            if (tag == JCF_CONSTANT_Double ||
                tag == JCF_CONSTANT_Long) {
                j++;
            }
            if (tag == JCF_CONSTANT_Fieldref ||
                tag == JCF_CONSTANT_Methodref ||
                tag == JCF_CONSTANT_InterfaceMethodref) {
                printf("Depdendencies - ");
		printf("tag is %u\n", tag);
                print_jcf_constant(jcf, (uint16_t) j, tag);
                printf("\n");
            }
        }
    }
...
process_jcf_body(struct jcf_state *jcf)
{
    ...
    // Read the body.
    if (fread(&access, sizeof(access), 1, jcf->f) != 1) {
        return (-1);
    }
    access = ntohs(access);

    if (fread(&this, sizeof(this), 1, jcf->f) != 1) {
        return (-1);
    }
    this = ntohs(this);

    if (fread(&super, sizeof(super), 1, jcf->f) != 1) {
        return (-1);
    }
    super = ntohs(super);
...
process_jcf_fields_and_methods_helper(struct jcf_state *jcf)
{
    ...
    printf("we are in the methods helper pre loop\n");
    // Read the count.
    ...
    for (i = 0; i < count; i++) {
        ...
        if (jcf->exports_flag &&
            ...
            if (print_jcf_constant(jcf, info.name_index,
                                   JCF_CONSTANT_Utf8) != 0) {
                printf("failed to print jcf info.name \n");
                return (-1);
            }
            printf(" ");
            if (print_jcf_constant(jcf, info.descriptor_index,
                                   JCF_CONSTANT_Utf8) != 0) {
                printf("failed to print jcf info.descriptor\n");
                return (-1);
            }
            printf("\n");
        }
        ...
        if (process_jcf_attributes(jcf) != 0) {
            printf("processing attributes failed...dumping \n");
            return (-1);
        }
    }
...
process_jcf_attributes(struct jcf_state *jcf)
{
    ...
    for (i = 0; i < attributes_count; i++) {
        ...
        if (fread(&attr_name_index, sizeof(attr_name_index), 1, jcf->f) != 1) {
            return (-1);
        }
        attr_name_index = ntohs(attr_name_index);

        // Read the attribute length.
        if (fread(&attr_length, sizeof(attr_length), 1, jcf->f) != 1) {
            return (-1);
        }
        attr_length = ntohl(attr_length);

        // Read the attribute data.
        for (j = 0; j < attr_length; j++) {
            if (fread(&attr, sizeof(attr), 1, jcf->f) != 1) {
                return (-1);
            }
        }
    }
...
main(int argc, char **argv)
{
    ...
    printf("Before Methods\n");
    // Process the JCF methods.
    err = process_jcf_methods(&jcf);
    printf("Went thru methods\n");
    if (err != 0)
        goto failed;
    printf("After Methods\n");

    printf("Before Final Attributes\n");
    // Process the JCF final attributes.
    err = process_jcf_attributes(&jcf);
    if (err != 0)
        goto failed;
    printf("After Final Attributes\n");

    printf("Checking for extra data\n");
    // Check for extra data.
    if (fgetc(jcf.f) != EOF) {
        err = -1;
        goto failed;
    }
    printf("Done checking for extra data\n");
